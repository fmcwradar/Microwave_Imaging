...
class prepare_radar_measurement:
    ...
    def __init__(self,path_csv,name,B,T_c,c0,number_of_ramps,total,f0,f1,windowing,ideal,swap_IQ,calibration,filtering,fc_low,fc_high,filterorder,distance_offset,save_last_measurement,background_subtraction,hilbert,execution_path):
        ...
        #For ideal data, it is not necessary to use more than one ramp.
        if self.ideal == True:
    ...
    def getdata(self):
        ...
        #Swap I and Q depending on the channel configuration of the oscilloscope.
        if self.swap_IQ == True:
            self.I,self.Q = self.Q,self.I
         
        #Calculate timestep.
        timestep = np.round(np.mean(np.diff(time)),8)
    ...
    def find_starting_point(self):
        ...
        if self.ideal == False:
            ...
            #For conventional radar measurements.
    ...
    def average_data_calculate_FTT(self):
        ...
        if self.calibration == True:
            ...
            if os.path.isfile(r"{0}\Pickle_Files\error_function_radar.pkl".format(self.execution_path)) == False:
                print("Error: No calibration file in folder!")
                print("Evaluation will be aborted!")
                exit(0)
        
            #Load error-function.
            with open(r"{0}\Pickle_Files\error_function_radar.pkl".format(self.execution_path), 'rb') as file: 
            ...
            with open(r"{0}\Calibration_Settings.txt".format(self.execution_path), 'r') as file:
            ...
            if self.windowing == True:
                mean_value_up_final = filtered_signal*np.hanning(self.samples_per_ramp)*error_function
        ...
        if self.calibration == False:
            ...
            #Apply window function if desired.
            if self.windowing == True:
                mean_value_up_final = filtered_signal*np.hanning(self.samples_per_ramp)
        ...
        #Apply background subtraction if desired.
        if self.background_subtraction == True:
        
            if self.calibration == True:
        
                with open(r"{0}\Pickle_Files\{1}_cal.pkl".format(self.execution_path,self.name), 'rb') as file: 
                    # Call load method to deserialze. 
                    background_spectrum = pickle.load(file)
 
            if self.calibration == False:
        
                with open(r"{0}\Pickle_Files\{1}_uncal.pkl".format(self.execution_path,self.name), 'rb') as file: 
                    # Call load method to deserialize. 
                    background_spectrum = pickle.load(file)
 
            spectrum_up -= background_spectrum
                    
        spectrum_up_corr = spectrum_up*np.exp(-1j*compensation_term)    #Multiply the range FFT with the phase correction term.
        
        #Save last measurement. Required if background subtraction shall be conducted. Different files are created depending on if calibration is enabled or not.
        if self.save_last_measurement == True:
            
            if self.calibration == True:
            
                filepath = r"{0}\Pickle_Files\{1}_cal.pkl".format(self.execution_path,self.name)
            
            if self.calibration == False:
                
                filepath = r"{0}\Pickle_Files\{1}_uncal.pkl".format(self.execution_path,self.name)
