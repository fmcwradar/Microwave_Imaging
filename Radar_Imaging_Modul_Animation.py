...
class radar_imaging:
    ...
    def calculate_image(self):
        ...
        #Create folder for results and log-file.
        ...
        for antenna_x in antenna_positions:
            ...
            test = self.image_matrix**2

            test_norm = test/np.max(np.abs(test))

            counter = counter + 1

            with open(r"{0}\image_radar_{1}.pkl".format(path,antenna_x), 'wb') as file: 
              
                # A new file will be created. 
                pickle.dump(test_norm, file) 
