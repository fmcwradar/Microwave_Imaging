...
class radar_measurement_evaluation:
    ...
    def __init__(self,path_csv,name,B,T_c,c0,number_of_ramps,total,f0,f1,windowing,ideal,swap_IQ,calibration,plotting,filtering,fc_low,fc_high,filterorder,measurement_offset,working_dir,):
    ...
    def getdata(self):
        ...
        filepath = str(r"{0}\{1}".format(self.path, self.name))
        columns = ['time', 'I', 'Q']
    ...
    def collect_ramp_data(self):
        ...
        for counter in range(1, self.number_of_ramps + 1):
            ...
            counter = counter + 1
            start = self.lineup[
                counter - 1]  # Starting point is at the transition from down-ramp to up-ramp. This why you have to "jump" one ramp forward to start with the negative slope.
            stop = start + self.samples_per_ramp
    ...
    def average_data_calculate_FTT(self):
        ...
        # Setting working directory (replaced by self.working_dir)
        # current_dir = os.path.dirname(__file__)
        # current_dir = f"{current_dir}\Multithread-Test"

        # Check if file exists.
        if self.calibration == "True":
            if os.path.isfile(r"{0}\Pickle Files\error_function_radar.pkl".format(self.working_dir)) == False:
                print("Error: No calibration file in folder!")
                exit(0)

            with open(r"{0}\Pickle Files\error_function_radar.pkl".format(self.working_dir), 'rb') as pickle_file:
                error_function = pickle.load(pickle_file)
        ...
        distance_corrected = np.linspace(self.distance[0], self.distance[len(self.distance) - 1],
                                         self.total)  # Adjust distance vector to the zero-padding.

        frequency_corrected = np.linspace(self.IF[0], self.IF[len(self.IF) - 1],
                                          self.total)  # Adjust frequency vector to the zero-padding.
        ...
        spectrum_up_corr = spectrum_up * np.exp(
            -1j * compensation_term)  # Multiply the range FFT with the phase correction term.
        ...
        # Modify name to account for offset.
        z_position = float(self.name.split('_')[0])
        rest_of_name = str(self.name.split('_')[1])
        z_position_corrected = z_position + self.measurement_offset
        corrected_name = "{0}_{1}".format(z_position_corrected, rest_of_name)
        # Save phase-compensated spectrum in pickle file.
        filepath = r"{0}\Pickle Files for 3D Imaging\{1}.pkl".format(self.working_dir, str(corrected_name[:-4]))
        ...
        with open(filepath, 'wb') as file:
            pickle.dump(self.spectrum_up, file)

        if corrected_name == "0.0_0.0.csv":
            # Save distance in pickle file.
            filepath = r"{0}\Pickle Files for 3D Imaging\distance.pkl".format(self.working_dir)

            if os.path.exists(filepath):
                os.remove(filepath)

            with open(filepath, 'wb') as file:
                pickle.dump(self.distance_corrected, file)
    ...
    def plot_results(self):
        ...
        #Plot IF signal.
        filepath = r'{0}\Results\Plot_IF_Signal\{1}.pdf'.format(self.working_dir,self.name)

        if os.path.exists(filepath):
            os.remove(filepath)

        plt.style.use(r"{0}\Style_MM.mplstyle".format(self.working_dir))
        ...
        #Plot IF-Spectrum.
        filepath = r'{0}\Results\Plot_IF_Spectrum\{1}.pdf'.format(self.working_dir,self.name)
