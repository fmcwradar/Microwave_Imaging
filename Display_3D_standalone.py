...
def update_rotation(event):
    global frame_counter, direction

    num_frames = 180
    step = 180 / num_frames

    if frame_counter < num_frames / 2:
        view1.camera.azimuth -= step * direction
    elif frame_counter < num_frames:
        view1.camera.azimuth += step * direction
    else:
        rotation_timer.stop()
...
# Create a Timer
rotation_timer = app.Timer(interval=1/60, connect=update_rotation, start=False)
...
def move_plane(event):
    ...
    if current_plane < 0:
    ...
    elif 0 < current_plane <= np.shape(vol)[np.where(plane.plane_normal == 1)[0][0]]:
        if (plane.plane_normal == [1, 0, 0]).all():
            plane.plane_position = plane.plane_position - [1, 0, 0]
        elif (plane.plane_normal == [0, 1, 0]).all():
            plane.plane_position = plane.plane_position - [0, 1, 0]
        ...
        plane.plane_position = (plane_start[0], plane_start[1], plane_start[2])
